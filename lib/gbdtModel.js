...
// ─── LightGBM inference (Track A) ────────────────────────────────────────────
//
// Reads models produced by scripts/train_lgbm.py (type: "lgbm").
// LightGBM stores trees in dump_model()['tree_info'] format:
//   - Internal nodes: { split_feature, threshold, left_child, right_child }
//   - Leaf nodes:     { leaf_value }
// Raw score = sum of leaf values across all trees (no separate base/bias term —
// LightGBM bakes the initial log-odds into the leaf values themselves).
// P(label=1) = sigmoid(raw_score).
//
// Verified: manual traversal == booster.predict(raw_score=True) to 8 decimal places.

function _traverseLgbm(node, x) {
  // Leaf detection: LightGBM dump nodes have 'leaf_value' key, internal nodes don't
  if (node.leaf_value !== undefined) return node.leaf_value;
  // Internal split: decision_type is always '<=' for standard GBDT
  return x[node.split_feature] <= node.threshold
    ? _traverseLgbm(node.left_child,  x)
    : _traverseLgbm(node.right_child, x);
}

function predictLgbm(model, x) {
  // model is the parsed JSON object from train_lgbm.py
  // model.model.tree_info = array of LightGBM tree dump objects
  const treeInfo = (model.model && model.model.tree_info) || model.trees || [];
  let raw = 0;
  for (const treeObj of treeInfo) {
    raw += _traverseLgbm(treeObj.tree_structure, x);
  }
  return sigmoid(raw);
}
