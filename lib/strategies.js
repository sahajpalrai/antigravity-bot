...
function evaluateStrategies(candles1m, candles5m, optParams, sessionRegime) {
  ...
  if (sessionRegime.meanReversionEnabled) {
    ...
    // Enforce 5m HTF Trend alignment: only buy support in uptrend (trend5m === 1), only short resistance in downtrend (trend5m === -1)
    const buyMeanReversion = closePrev < bb.lower[lastIndex1m - 1] && c1m.close > bb.lower[lastIndex1m] && rsi1m[lastIndex1m] < (optParams.rsiOversold || 35) && trend5m === 1;
    const sellMeanReversion = closePrev > bb.upper[lastIndex1m - 1] && c1m.close < bb.upper[lastIndex1m] && rsi1m[lastIndex1m] > (optParams.rsiOverbought || 65) && trend5m === -1;

    if (buyMeanReversion) {
      return {
        shouldBuy: true,
        shouldSell: false,
        reason: `Bollinger Lower Band Close & Reversion with RSI oversold (${rsi1m[lastIndex1m].toFixed(1)}) aligned with 5m Bullish Trend`,
    ...
    if (sellMeanReversion) {
      return {
        shouldBuy: false,
        shouldSell: true,
        reason: `Bollinger Upper Band Close & Reversion with RSI overbought (${rsi1m[lastIndex1m].toFixed(1)}) aligned with 5m Bearish Trend`,
        strategyName: 'Bollinger Bands Reversion',
        atr: atr1m[lastIndex1m]
      };
    }
  ...
  if (sessionRegime.meanReversionEnabled) {
    ...
    const stochCrossUp = stochK > stochD && stochKPrev <= stochDPrev && stochK < (optParams.stochOversold || 25);
    const stochCrossDown = stochK < stochD && stochKPrev >= stochDPrev && stochK > (optParams.stochOverbought || 75);

    const rsiBullish = rsi1m[lastIndex1m] < (optParams.rsiOversold || 38);
    const rsiBearish = rsi1m[lastIndex1m] > (optParams.rsiOverbought || 62);

    if (stochCrossUp && rsiBullish && trend5m === 1) {
      return {
        shouldBuy: true,
        shouldSell: false,
        reason: `Stochastic Crossover in oversold, RSI confluence, aligned with 5m Bullish Trend`,
        strategyName: 'Stoch & RSI Confluence',
        atr: atr1m[lastIndex1m]
      };
    }
    if (stochCrossDown && rsiBearish && trend5m === -1) {
      return {
        shouldBuy: false,
        shouldSell: true,
        reason: `Stochastic Crossover in overbought, RSI confluence, aligned with 5m Bearish Trend`,
        strategyName: 'Stoch & RSI Confluence',
        atr: atr1m[lastIndex1m]
      };
    }
