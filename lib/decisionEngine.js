...
function decide(symbol, candles, opts = {}) {
  ...
  // RTH-MIRROR-ETH (Settings toggle): when on, RTH uses the ETH session's model +
  // threshold + quality-floor + session-quality margin. Display + the per-symbol
  // session-trading gate stay on the REAL session (fv.session) to avoid confusion.
  const _ms = (_loadRthMirror() && fv.session === 'RTH') ? 'ETH' : fv.session;

  // â”€â”€ CHECK 4: Quality gate â€” load deployed models â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const longEntry  = _loadModel(symbol, _ms, regimeInfo.regime, 'long');
  const shortEntry = _loadModel(symbol, _ms, regimeInfo.regime, 'short');
  ...
  const _sqBonus = _loadSessionQuality()[`${symbol}_${_ms}`] || 0;
