...
function _loadFloors(sym) {
  try {
    if (fs.existsSync(_FLOORS_FILE)) {
      ...
      // BACKTEST-ONLY override (env BT_FLOOR_RTH / BT_FLOOR_ETH) — uniform WR floors
      // for the "stricter quality gate" A/B. Applies to ALL symbols (including ES's
      // looser per-symbol floors). Inert live (env never set by the server).
      if (process.env.BT_FLOOR_RTH) out.rth = parseFloat(process.env.BT_FLOOR_RTH);
      if (process.env.BT_FLOOR_ETH) out.eth = parseFloat(process.env.BT_FLOOR_ETH);
      return out;
    }
...
function recordTradeResult({ symbol, session, regime, direction, pnl, exitTime, entryPrice }) {
  ...
  // Daily P&L roll-up is NO LONGER accumulated here — the reconstructed ledger
  // under-counted (missed closes) and let the -$1500 cap overshoot to -$2,642.
  // seedDailyPnLFromNt8() now mirrors NT8's true realized on every METRICS and is
  // the single source for the daily-loss cap. recordTradeResult keeps only the
  // per-bucket loss-streak / re-entry guards below.
...
// AUTHORITATIVE daily-loss tracker — called on EVERY NT8 METRICS (not just once).
// The bot's reconstructed trade ledger UNDER-counts the true loss (it misses some
// closes — that let the -$1500 cap overshoot to -$2,642 on NQ 2026-06-03). This
// mirrors NT8's ACTUAL realized P&L: anchor the day's starting realized, then
// today's realized = current realized - anchor. Keyed by family/mini symbol so the
// cap works in MICRO mode too. Persisted so a mid-day restart preserves the day.
function seedDailyPnLFromNt8(symbol, nt8RealizedPnL) {
  if (!symbol || typeof nt8RealizedPnL !== 'number') return;
  const s = _safetyState();
  const todayKey = _etDateKey();  // ET-based so ETH after midnight UTC stays on the right day
  const key = familyMiniSymbol(symbol);
  let rec = s.dailyPnL[key];
  if (!rec || rec.date !== todayKey) {
    // New ET day (or first-ever capture). Anchor the day's starting realized so the
    // day's delta begins at 0. EXCEPTION: a first capture with an already-negative
    // realized is conservatively treated as today's loss (anchor 0) so the loss cap
    // can never be under-counted after a mid-day (re)start.
    const anchor = rec ? nt8RealizedPnL : (nt8RealizedPnL < 0 ? 0 : nt8RealizedPnL);
    s.dailyPnL[key] = { date: todayKey, dayStartRealized: anchor, pnl: nt8RealizedPnL - anchor };
    _persistSafety();
    return;
  }
  if (rec.dayStartRealized == null) rec.dayStartRealized = nt8RealizedPnL - (rec.pnl || 0); // backfill legacy records
  rec.pnl = nt8RealizedPnL - rec.dayStartRealized;   // true today realized, straight from NT8
  _persistSafety();
}

// Clear the daily-loss halt for a symbol — re-anchors today's counter to the CURRENT
// realized P&L (today's loss -> 0). Called by the per-symbol "Reset Cap" button so the
// bot resumes trading that symbol until the cap is hit again. Does NOT touch balance,
// cumulative P&L, history, or the drawdown floor.
function resetDailyPnL(symbol, currentRealized) {
  const s = _safetyState();
  const key = familyMiniSymbol(symbol);
  const todayKey = _etDateKey();
  const prev = s.dailyPnL[key];
  const anchor = (typeof currentRealized === 'number')
    ? currentRealized
    : (prev ? (prev.dayStartRealized || 0) + (prev.pnl || 0) : 0);  // fall back to last-known realized
  s.dailyPnL[key] = { date: todayKey, dayStartRealized: anchor, pnl: 0 };
  _persistSafety();
  return { ok: true, symbol, dayStartRealized: anchor, pnl: 0 };
}
...
function _loadDisabledBundles() {
  // Backtest-only override: IGNORE_DISABLED=1 returns no disables so a backtest can
  // measure a currently-disabled bundle's true performance (with/without guards)
  // WITHOUT editing the live disabled_bundles.json. Never set in production.
  if (process.env.IGNORE_DISABLED === '1') return {};
...
function _loadExhaustGuard() {
  // EXHAUST_SYMBOLS env (e.g. "NQ" or "ES,NQ") overrides which symbols the guard
  // applies to — used for A/B backtests. Defaults to the file's symbols, else ['ES'].
  const envSyms = process.env.EXHAUST_SYMBOLS ? process.env.EXHAUST_SYMBOLS.split(',').map(s => s.trim()) : null;
  if (process.env.EXHAUST_GUARD === '1') return { enabled: true,  adxMin: 30, vwapMin: 4, symbols: envSyms || ['ES'] };
  if (process.env.EXHAUST_GUARD === '0') return { enabled: false, adxMin: 30, vwapMin: 4, symbols: envSyms || ['ES'] };
  ...
  try {
    ...
    _egCache = { enabled: !!o.enabled, adxMin: o.adxMin != null ? o.adxMin : 30, vwapMin: o.vwapMin != null ? o.vwapMin : 4,
                 symbols: envSyms || (Array.isArray(o.symbols) ? o.symbols : ['ES']) };  // explicit [] = guard nobody
  } catch (e) {
    _egCache = { enabled: false, adxMin: 30, vwapMin: 4, symbols: envSyms || ['ES'] };
  }
  _egCacheMs = Date.now();
  return _egCache;
}
...
// RTH-MIRROR-ETH toggle: when on, RTH uses the ETH session's models/thresholds/floors.
// env RTH_MIRROR_ETH=1/0 wins (backtest); else models/rth_mirror_eth.json {enabled},
// hot-reloaded (10s). OFF by default — keeps the selective RTH config.
let _rmCache = null, _rmCacheMs = 0;
function _loadRthMirror() {
  if (process.env.RTH_MIRROR_ETH === '1') return true;
  if (process.env.RTH_MIRROR_ETH === '0') return false;
  if (_rmCache !== null && (Date.now() - _rmCacheMs) < 10000) return _rmCache;
  try { _rmCache = !!JSON.parse(fs.readFileSync(path.join(MODELS_DIR, 'rth_mirror_eth.json'), 'utf-8')).enabled; }
...
function decide(symbol, candles, opts = {}) {
  ...
  // ── Read-only CHOP INDICATOR (informational only — NEVER blocks a trade) ──
  // Live Kaufman efficiency ratio over the last 20 bars: ~0 = whipsaw/chop,
  // ~1 = clean trend. Plus the volatility rank. Surfaced on the dashboard so the
  // operator can SEE choppy conditions (like today's -$2,642 day) without the bot
  // cutting any trade. This is NOT the chop guard (which stays OFF, data-proven).
  liveFeatures.chopER     = _efficiencyRatio(candles, 20);
  liveFeatures.atrPctile  = fv.values[fv.names.indexOf('atr_percentile')];
  liveFeatures.chopStatus = liveFeatures.chopER < 0.34 ? 'CHOPPY'
                          : (liveFeatures.chopER < 0.50 ? 'MIXED' : 'CLEAN');
  ...
  // NOTE: always the REAL session model (RTH model in RTH). The RTH-mirror toggle only
  // lowers the THRESHOLD (below), keeping the in-distribution RTH brain — NOT the old
  // model-swap, which ran the ETH brain in RTH and lost ~$181k/yr.
  ...
  // RTH-MIRROR toggle: lower the RTH firing threshold to the matching ETH bundle's level
  // (e.g. NQ TREND_DOWN short 0.72→0.52) so the in-distribution RTH model fires MORE in
  // the day session. Keeps the RTH brain — only the bar drops. OFF by default.
  if (_loadRthMirror() && fv.session === 'RTH') {
    if (longUsable)  { const e = _loadModel(symbol, 'ETH', regimeInfo.regime, 'long');  if (e && e.threshold) longTh  = Math.min(longTh,  e.threshold); }
    if (shortUsable) { const e = _loadModel(symbol, 'ETH', regimeInfo.regime, 'short'); if (e && e.threshold) shortTh = Math.min(shortTh, e.threshold); }
  }
  ...
  try {
    ...
    if (profile && profile.runtimeThresholdCap != null) {
      // env THRESHOLD_CAP overrides for A/B backtests only (no live effect unless set).
      const cap = process.env.THRESHOLD_CAP != null ? parseFloat(process.env.THRESHOLD_CAP) : profile.runtimeThresholdCap;  // uniform RTH/ETH
      if (longUsable  && longTh  > cap) longTh  = cap;
      if (shortUsable && shortTh > cap) shortTh = cap;
    }
  } catch (e) { /* use bundle-baked thresholds */ }

  // â”€â”€ CHECK 6: Daily P&L cap (one cap per symbol, editable in Settings) â”€â”€â”€â”€â”€â”€
  // models/daily_loss_caps.json: perSymbol[FAMILY] = dollars (e.g. NQ:1500), else
  // default.single. When a symbol's realized loss for the day reaches the cap it
  // stops opening new trades until the next futures day OR the operator clicks the
  // per-symbol "Reset Cap" button (POST /api/reset-daily-cap -> resetDailyPnL).
  ...
  const _capCfg = _loadDailyCaps();
  const _pc     = _capCfg.perSymbol && _capCfg.perSymbol[_ddKey.replace('=F', '')];
  const _capAmt = (typeof _pc === 'number' && _pc > 0) ? _pc
                : ((_capCfg.default && _capCfg.default.single) || 1500);
  if (todayPnL <= -_capAmt) {
    return { action: 'FLAT', reason: `daily P&L cap hit â€” ${symbol} ${todayPnL.toFixed(0)} (cap -${_capAmt})`,
  ...
  // BACKTEST-ONLY hook (env CT_MIN_PROB, e.g. 0.60) — conviction floor for
  // COUNTER-TREND entries: TREND_DOWN longs / TREND_UP shorts must clear this
  // absolute probability. Motivated by 2026-06-10 overnight: TREND_DOWN longs
  // at p=0.54-0.59 were the stop-outs, p>=0.65 were the winners. Inert live
  // (env never set by the server).
  const _ctMin = process.env.CT_MIN_PROB ? parseFloat(process.env.CT_MIN_PROB) : 0;
  if (_ctMin > 0) {
    if (regimeInfo.regime === 'TREND_DOWN' && longProb  < _ctMin) longBundleFire  = false;
    if (regimeInfo.regime === 'TREND_UP'   && shortProb < _ctMin) shortBundleFire = false;
  }

  // BACKTEST-ONLY hook (env MIN_PROB) — absolute conviction floor on ALL entries,
  // every regime/direction. Targets the threshold-skimming losers (the 2026-06-11
  // ES VOL_EXP long at p=0.53, the 2026-06-10 NQ longs at p=0.54-0.59). Only bites
  // low-threshold bundles; high-threshold ones already clear it. Inert live.
  const _minP = process.env.MIN_PROB ? parseFloat(process.env.MIN_PROB) : 0;
  if (_minP > 0) {
    if (longProb  < _minP) longBundleFire  = false;
    if (shortProb < _minP) shortBundleFire = false;
  }

  // BACKTEST-ONLY hook (env VOLEXP_MIN_PROB) — conviction floor for VOL_EXPANSION
  // entries only (the regime of today's ES loser). Inert live.
  const _veMin = process.env.VOLEXP_MIN_PROB ? parseFloat(process.env.VOLEXP_MIN_PROB) : 0;
  if (_veMin > 0 && regimeInfo.regime === 'VOL_EXPANSION') {
    if (longProb  < _veMin) longBundleFire  = false;
    if (shortProb < _veMin) shortBundleFire = false;
  }

  // BACKTEST-ONLY hook (env DEEP_ASIA_MARGIN, e.g. 0.03) — extra threshold margin
  // during the deep-Asian window 10 PM-2 AM ET (02:00-06:00 UTC in EDT), where
  // research + the 2026-06-10 overnight bleed show elevated false-breakout rates.
  // Soft gate (higher bar), not a block. Inert live.
  const _asiaM = process.env.DEEP_ASIA_MARGIN ? parseFloat(process.env.DEEP_ASIA_MARGIN) : 0;
  if (_asiaM > 0) {
    const _uh = new Date(fv.ts || Date.now()).getUTCHours();
    if (_uh >= 2 && _uh < 6) {
      if (longProb  < longTh  + MARGIN + _sqBonus + _asiaM) longBundleFire  = false;
      if (shortProb < shortTh + MARGIN + _sqBonus + _asiaM) shortBundleFire = false;
    }
  }
  ...
  if (_eg.enabled && regimeInfo.regime === 'TREND_UP' && _eg.symbols.some(p => symbol.startsWith(p))) {
...
// Per-trade dollar-risk stop cap. env SL_DOLLAR_CAP wins (backtest A/B); else
// models/stop_cap.json {enabled, maxDollar}, hot-reloaded (10s). 0 = off.
let _scCache = null, _scCacheMs = 0;
function _loadStopCap() {
  if (process.env.SL_DOLLAR_CAP) return parseFloat(process.env.SL_DOLLAR_CAP) || 0;
  if (_scCache !== null && (Date.now() - _scCacheMs) < 10000) return _scCache;
  try {
    const o = JSON.parse(fs.readFileSync(path.join(MODELS_DIR, 'stop_cap.json'), 'utf-8'));
    _scCache = (o.enabled && o.maxDollar > 0) ? o.maxDollar : 0;
  } catch (e) { _scCache = 0; }
  _scCacheMs = Date.now();
  return _scCache;
}

// Session-scoped daily-loss caps. models/daily_loss_caps.json:
//   { default:{single:1500}, perSymbol:{ NQ:{ETH:1000, RTH:500} } }
// A symbol with ETH+RTH keys gets SEPARATE budgets per session so an overnight
// ETH drawdown can't pre-block the RTH day session (ETH+RTH sum to the old single
// cap, so total daily risk is unchanged). Symbols without a split use {single}.
// Hot-reloaded (10s). Default preserves the legacy single -1500 for every symbol.
let _dlcCache = null, _dlcCacheMs = 0;
function _loadDailyCaps() {
  if (_dlcCache && (Date.now() - _dlcCacheMs) < 10000) return _dlcCache;
  try {
    _dlcCache = JSON.parse(fs.readFileSync(path.join(MODELS_DIR, 'daily_loss_caps.json'), 'utf-8'));
  } catch (e) { _dlcCache = { default: { single: 1500 }, perSymbol: {} }; }
  _dlcCacheMs = Date.now();
  return _dlcCache;
}

// Resolve the effective daily cap ($) for each main symbol — for the Settings UI.
function getDailyCaps() {
  const cfg = _loadDailyCaps();
  const def = (cfg.default && cfg.default.single) || 1500;
  const out = { default: def, perSymbol: {} };
  ['NQ', 'ES', 'CL', 'GC'].forEach(s => {
    const v = cfg.perSymbol && cfg.perSymbol[s];
    out.perSymbol[s] = (typeof v === 'number' && v > 0) ? v : def;
  });
  return out;
}

// Set the per-symbol daily cap ($) from the Settings field. Writes the file and
// busts the cache so the new cap is live on the next decision (no restart).
function setDailyCap(symbol, amount) {
  const amt = parseFloat(amount);
  if (!symbol || !(amt > 0)) return { ok: false, error: 'symbol + positive amount required' };
  const fam = String(symbol).replace('=F', '');
  let cfg;
  try { cfg = JSON.parse(fs.readFileSync(path.join(MODELS_DIR, 'daily_loss_caps.json'), 'utf-8')); }
  catch (e) { cfg = { default: { single: 1500 }, perSymbol: {} }; }
  if (!cfg.perSymbol) cfg.perSymbol = {};
  cfg.perSymbol[fam] = amt;
  cfg._updatedAt = _etDateKey();
  try { fs.writeFileSync(path.join(MODELS_DIR, 'daily_loss_caps.json'), JSON.stringify(cfg, null, 2)); }
  catch (e) { return { ok: false, error: 'write failed: ' + e.message }; }
  _dlcCache = null;  // bust cache -> next decision reads the new cap
  return { ok: true, symbol: fam, amount: amt };
}
...
function _buildEntry(action, symbol, fv, prob, threshold, regimeInfo, slAtrMult, tpAtrMult, snapshot) {
  ...
  // Per-trade DOLLAR-RISK CAP — bounds worst-case loss/contract so a high-ATR (high-vol)
  // stop can't balloon to -$2,000+. Caps ONLY the stop distance; TP keeps its ATR size
  // (so R:R improves). Binds only when the ATR stop exceeds the cap (i.e. tail trades).
  // env SL_DOLLAR_CAP (backtest) or models/stop_cap.json {enabled,maxDollar} (live).
  if (exitMode === 'ATR') {
    const _slCap = _loadStopCap();
    if (_slCap > 0) {
      const _PV = { 'NQ=F':20,'ES=F':50,'CL=F':1000,'GC=F':100,'MNQ=F':2,'MES=F':5,'MCL=F':100,'MGC=F':10 };
      const pv = _PV[symbol] || 20;
      const maxDist = _slCap / pv;
      if (slDistance > maxDist) { slDistance = maxDist; exitMode = 'ATR+CAP'; }
    }
  }
  return {
    action,
