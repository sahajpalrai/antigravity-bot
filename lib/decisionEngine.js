...
// Deep Asian session gate — 10 PM to 2 AM ET.
...
//   • NQ/ES volume drops to ~10% of RTH during this window
//   • Spread widens to 0.5–1.25 pts (NQ)
//   • 40% false-breakout rate for NQ overnight; worst in deep Asian hours
//   • V6 overnight -$1,200+ loss on 2026-05-18 was T3 drought forcing trades
//     exactly into this window (8 PM ET) against a 160-pt NQ bounce.
// ETH signals are allowed in the London pre-open (2–5 AM ET) and US evening
// (6–10 PM ET) windows, which have measurably better liquidity.
function _isDeepAsianWindow(nowMs) {
  ...
  const etHour  = (d.getUTCHours() - offset + 24) % 24;  // 0–23 ET
  return etHour >= 22 || etHour < 2;                      // 10 PM – 1:59 AM ET
}
...
// ─── Percentile threshold — self-calibrating per bundle ──────────────────────
...
// at 0.49–0.52), the 70th percentile of recent scores drops to ~0.515 — meaning
...
const _SCORE_BUF_FLOOR  = 0.48;   // absolute floor — never fire below this
const _scoreBuffers     = new Map();  // bundleKey → number[]
...
// Returns the adaptive threshold if buffer has ≥ 20 samples, else null (use static).
...
function decide(symbol, candles, opts = {}) {
  // Pass symbol so buildFeatures uses correct session boundaries and
  // instrument-specific features (CL: EIA window, GC: London fix, etc.)
  ...
  // ── DEEP ASIAN SESSION BLOCK (10 PM – 2 AM ET) ───────────────────────
  // Research finding 2026-05-28: NQ/ES volume ~10% of RTH, spread 0.5–1.25 pts,
  // 40% false-breakout rate in this specific window. Every professional source
  // (Edgeful, TRADEPRO, prop firm docs) and the V6 loss audit all point here.
  // London pre-open (2–5 AM ET) and US evening (6–10 PM ET) retain enough
  // institutional participation to be worth trading — this window does not.
  if (session === 'ETH' && _isDeepAsianWindow()) {
    return {
      action: 'FLAT',
      reason: 'deep_asian_block (10 PM–2 AM ET — low liquidity, high false-breakout rate)',
      regime: 'ETH_DEEP_ASIAN',
      session,
      liveFeatures
    };
  }
  ...
  // ── ROLLING SCORE BUFFER UPDATE ───────────────────────────────────────
  // Record every raw probability into the per-bundle circular buffer so the
  // percentile threshold has live distribution data to adapt against.
  const _longBundleKey  = `${symbol}|${fv.session}|${regimeInfo.regime}|long`;
  const _shortBundleKey = `${symbol}|${fv.session}|${regimeInfo.regime}|short`;
  if (longUsable)  _recordScore(_longBundleKey,  longProb);
  if (shortUsable) _recordScore(_shortBundleKey, shortProb);
  ...
  // ── PERCENTILE THRESHOLD (self-calibrating) ────────────────────────────
  // After the static threshold and adaptive streak adjustment, apply the
  // percentile gate. When the ETH distribution compresses to 0.49–0.52 and
  // the static threshold would block every bar, the 70th-percentile of the
  // last 100 scores drops to ~0.511 — threshold adapts automatically.
  // Rule: effective threshold = min(current_th, percentile_th), clamped ≥ 0.48.
  // This permanently replaces manual ETH patch files — the system self-heals.
  // Only applied once we have 20+ scores (warm-up window to avoid premature fires).
  const _longPctTh  = _percentileTh(_longBundleKey);
  const _shortPctTh = _percentileTh(_shortBundleKey);
  let longPctAdapted = false, shortPctAdapted = false;
  if (_longPctTh  !== null && _longPctTh  < longTh)  { longTh  = _longPctTh;  longPctAdapted  = true; }
  if (_shortPctTh !== null && _shortPctTh < shortTh) { shortTh = _shortPctTh; shortPctAdapted = true; }
  ...
  // CHOP gets a small extra wall — the quality gate (60%/62% WR floor) is the main
  // defense; this adds minor friction so marginal CHOP signals require slightly
  // more probability than trend signals. Kept small (+0.02) so that quality-passing
  // CHOP bundles (≥60% WR) can still fire — ETH probs max ~0.52-0.54 so anything
  // over +0.04 would silence even the best CHOP specialists.
  const _regimeName = regimeInfo && regimeInfo.regime ? regimeInfo.regime : 'CHOP';
  const _chopWall   = _regimeName === 'CHOP'          ? 0.02 :
                      _regimeName === 'VOL_EXPANSION'  ? 0.01 : 0.00;
  const baseMargin = (session === 'ETH' ? 0.01 : 0.02) + _chopWall;
  ...
  // fire AT the cap exactly — but ALWAYS apply the CHOP/VOL_EXPANSION wall so that
  // noisy regimes never sneak through on a low cap.
  // When using the native threshold (not capped), add the full baseMargin (which
  // already includes the regime wall) to stay off the trained edge.
  const longBundleFire  = longUsable  && longProb  >= longTh  + (longWasCapped  ? _chopWall : baseMargin);
  const shortBundleFire = shortUsable && shortProb >= shortTh + (shortWasCapped ? _chopWall : baseMargin);
  ...
  if (longPctAdapted)  blockReasons.push(`long percentile-th adapted: ${_longPctTh.toFixed(3)} (score distribution compressed)`);
  if (shortPctAdapted) blockReasons.push(`short percentile-th adapted: ${_shortPctTh.toFixed(3)} (score distribution compressed)`);
