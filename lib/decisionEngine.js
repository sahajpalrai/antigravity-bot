...
function decide(symbol, candles, opts = {}) {
  ...
  // Run GBDT predictions
  let longProb = 0, shortProb = 0;
  let longTh = 0.65, shortTh = 0.65;
  if (longUsable)  { longProb  = predict(longEntry.model,  fv.values); longTh  = longEntry.threshold; }
  if (shortUsable) { shortProb = predict(shortEntry.model, fv.values); shortTh = shortEntry.threshold; }
