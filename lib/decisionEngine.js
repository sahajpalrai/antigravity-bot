...
// Clear the daily-loss halt for a symbol by re-anchoring today's counter to the
// CURRENT realized P&L (today's loss -> 0). Called when the operator resets the
// drawdown so the bot resumes trading that symbol until a limit is hit again.
// Also clears the RTH session anchor so RTH re-anchors fresh on its next bar.
function resetDailyPnL(symbol, currentRealized) {
  ...
  s.dailyPnL[key] = { date: todayKey, dayStartRealized: anchor, pnl: 0 };  // rthStartPnl dropped -> re-anchors
  _persistSafety();
...
function decide(symbol, candles, opts = {}) {
  ...
  // â”€â”€ CHECK 6: Daily P&L floor â€” session-scoped â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // A single -1500 day cap let an overnight ETH drawdown spend the WHOLE budget
  // before RTH opened (one 2-contract NQ stop ~ -$1666 > -1500), so RTH never traded.
  // With a per-session split (e.g. NQ ETH -1000 / RTH -500), RTH gets a FRESH budget
  // anchored at the RTH open, while ETH+RTH still sum to the old -1500 total daily risk.
  ...
  const _rec = (guards.dailyPnL[_ddKey] && guards.dailyPnL[_ddKey].date === todayKey) ? guards.dailyPnL[_ddKey] : null;
  const todayPnL = _rec ? _rec.pnl : 0;
  const _capCfg = _loadDailyCaps();
  const _famKey = _ddKey.replace('=F', '');
  const _split  = _capCfg.perSymbol && _capCfg.perSymbol[_famKey];
  let _sessLoss = todayPnL, _capAmt, _sessLabel;
  if (_split && _split.ETH != null && _split.RTH != null && _rec) {
    if (fv.session === 'RTH') {
      // Anchor the RTH budget to the day's P&L the first time RTH trades today,
      // so RTH risks only its own slice regardless of the overnight ETH result.
      if (_rec.rthStartPnl == null) { _rec.rthStartPnl = _rec.pnl; _persistSafety(); }
      _sessLoss = _rec.pnl - _rec.rthStartPnl;     // loss since RTH opened
      _capAmt = _split.RTH; _sessLabel = 'RTH';
    } else {
      _sessLoss = _rec.pnl;                          // overnight/ETH portion = day total before RTH
      _capAmt = _split.ETH; _sessLabel = 'ETH';
    }
  } else {
    _capAmt = (_capCfg.default && _capCfg.default.single) || 1500;  // legacy single cap
    _sessLabel = 'day';
  }
  if (_sessLoss <= -_capAmt) {
    return { action: 'FLAT', reason: `daily P&L cap hit â€” ${symbol} ${_sessLabel} ${_sessLoss.toFixed(0)} (cap -${_capAmt})`,
