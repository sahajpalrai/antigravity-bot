...
// ── Regime allow-list (hot-reload) ────────────────────────────────────────────
// Gate 2's full-year edge is break-even (PF ~1.01) because most regimes are
// coin-flips and some bleed (RTH_CHOP, ETH_TREND_UP). Only RTH_TREND_UP and
// ETH_TREND_DOWN are net-positive on BOTH NQ and ES. This allow-list restricts
// Gate 2 to those regimes so the SHADOW forward-validates the profitable subset.
// Keyed by `${session}_${regime}` (e.g. "RTH_TREND_UP"). enabled:false = trade
// all tradeable regimes (old behavior).
const GATE2_ALLOWLIST_FILE = path.join(__dirname, '..', 'gate2', 'regime_allowlist.json');
let _allowCache = null, _allowCacheMs = 0;

function _loadRegimeAllowlist() {
  if (_allowCache && (Date.now() - _allowCacheMs) < CONFIG_TTL_MS) return _allowCache;
  try {
    const j = JSON.parse(fs.readFileSync(GATE2_ALLOWLIST_FILE, 'utf-8'));
    _allowCache = { enabled: j.enabled === true, allow: Array.isArray(j.allow) ? j.allow : [] };
  } catch (e) {
    _allowCache = { enabled: false, allow: [] };  // file missing → no restriction
  }
  _allowCacheMs = Date.now();
  return _allowCache;
}
...
// nowMs is optional — defaults to wall clock (live). Backtests pass the bar
// timestamp so these gates evaluate against the historical bar, not "now".
...
async function decide2(symbol, candles, opts = {}) {
  ...
  // Wall clock for live; injected bar time for backtests (opts.nowMs).
  const nowMs = opts.nowMs != null ? opts.nowMs : Date.now();

  // ── Session gates ──
  if (session === 'ETH' && _isDeepAsianWindow(nowMs)) {
  ...
  // ── Regime allow-list (forward-validation of the profitable subset) ──
  const _allow = _loadRegimeAllowlist();
  if (_allow.enabled && !_allow.allow.includes(`${session}_${regimeInfo.regime}`)) {
    return {
      action: 'FLAT', reason: `regime ${session}_${regimeInfo.regime} not in Gate 2 allow-list`,
      regime: regimeInfo.regime, session, gate: 'gate2'
    };
  }
  ...
  if (mlTrained && !fvgBypass) {
    ...
    // Consensus floor: a 2+ pattern consensus may override a WEAK ML block
    // (0.50 <= prob < threshold), but NOT when ML actively leans the other way
    // (prob < 0.50). Without this floor the filter was bypassed ENTIRELY —
    // the 2026-05-30 3yr backtest showed Gate 2 firing at prob 0.26-0.34,
    // 33.5% WR, PF 1.00 (a coin-flip surviving only on 2:1 R:R). The original
    // comment intended "prob >= 0.50" but the code never enforced it.
    const CONSENSUS_FLOOR = 0.50;
    if (prob < CONSENSUS_FLOOR) {
      return {
        action: 'FLAT',
        reason: `ML against (${signal} prob=${prob.toFixed(2)}<0.50) — 2-pattern consensus cannot override`,
        regime: regimeInfo.regime, session, gate: 'gate2',
        patterns: patternResult.patterns
      };
    }
    // Soft block survivor: 0.50 <= prob < threshold with 2+ patterns — allowed.
    if (prob < mlThreshold && agreeing >= 2) {
      console.log(`[Gate2] ${signal} passes by 2-pattern consensus (prob=${prob.toFixed(2)}>=0.50, agreeing=${agreeing})`);
    }
  }
