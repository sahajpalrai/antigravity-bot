...
function classifyRegime(featureVector, thresholds = DEFAULT_THRESHOLDS) {
  ...
  // 2. Price-slope momentum check — runs BEFORE the CHOP squeeze gate.
  //    A bar with strong directional momentum (ret_20 >= 0.10% + EMA50 agrees)
  //    is a trend even if ADX hasn't caught up yet and bandwidth is tight.
  //    This prevents the CHOP gate from silencing high-momentum bars
  //    (e.g. NQ ADX=14 but MACD=11 and bb_z=2 — clearly breaking out).
  const ret20 = f.ret_20 != null ? f.ret_20 : 0;
  if (Math.abs(ret20) >= t.slope_tiebreaker_pct && f.adx >= 12) {
    if (ret20 > 0 && f.ema50_dist > 0) {
      return {
        regime: 'TREND_UP',
        reason: `momentum ret20=${(ret20*100).toFixed(2)}% adx=${f.adx.toFixed(1)}`,
        confidence: 0.5
      };
    }
    if (ret20 < 0 && f.ema50_dist < 0) {
  ...
  // 3. Chop / squeeze — no edge, sit out
  ...
  // 4. Trend regimes — 2-of-3 EMA-stack vote (relaxed from triple-stack).
  ...
  const stackDownHits = (f.ema9_21_gap < -t.ema_stack_threshold ? 1 : 0) +
                        (f.ema21_50_gap < -t.ema_stack_threshold ? 1 : 0) +
                        (f.ema50_dist <  0 ? 1 : 0);
  // Lead-pair (ema9_21_gap) must also clear the threshold — a near-zero gap
  // means ema9≈ema21 (crossover imminent) and is not a reliable trend signal.
