...
const DEFAULT_THRESHOLDS = {
  adx_trend_RTH: 22,        // RTH: ADX 22 — matches trainer's _regime() threshold exactly
  adx_trend_ETH: 22,        // ETH: unified to 22 to eliminate training/live distribution mismatch
  adx_trend: 22,            // legacy / non-session-aware default
  bb_bandwidth_chop: 0.012,
  bb_bandwidth_expansion: 0.030,
  atr_percentile_expansion: 0.85,
  ema_stack_threshold: 0.0003,           // 3 bps (was 5 bps — let EMA50 lag count)
  ...
  slope_tiebreaker_pct: 0.0020           // 0.20% over 20 bars
};
...
//
// To re-enable: comment out the CHOP exclusion below after achieving
// 60%+ live WR over ≥20 trades in paper mode with raised threshold.
function isTradeable(regime) {
  return regime === 'TREND_UP' || regime === 'TREND_DOWN' || regime === 'VOL_EXPANSION';
}
