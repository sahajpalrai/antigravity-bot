...
// Returns the active contract for a family, based on current contractMode.
// Example: family='NQ', mode='MICRO' → 'MNQ=F'
function activeContractFor(symbolOrFamily) {
  ...
  const targetList = portfolio.contractMode === 'MICRO' ? MICRO_SYMBOLS : MINI_SYMBOLS;
...
function loadPortfolioState() {
  ...
  try {
    ...
    for (const sym of ALL_SYMBOLS) {
      ...
      if (portfolio.accounts[sym].enabled === undefined) portfolio.accounts[sym].enabled = true;
      if (!portfolio.accounts[sym].accountNumber) portfolio.accounts[sym].accountNumber = defaultAccountNumber(sym);
    }
...
// Returns the currently active mini OR micro symbols based on contractMode
function activeSymbols() {
  return portfolio.contractMode === 'MICRO' ? MICRO_SYMBOLS : MINI_SYMBOLS;
}
