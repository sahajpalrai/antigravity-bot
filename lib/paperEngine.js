...
// Per-symbol trailing-drawdown default. NQ mini's stops (~$1-1.5k) are large vs a
// $2,500 trailing DD, so after a winning run ratchets the floor up, a normal
// pullback leaves too little room and the sizer refuses NQ (it "wins itself" into
// a lockout). A $6,000 buffer keeps NQ tradeable. Micros (1/10th risk) + ES/CL/GC
// keep the default. (Audit fix 2026-06-03; also settable live via /api/drawdown-amount.)
// Per-symbol trailing-drawdown buffer. $2,500 (default) is too tight for index-future
// volatility: a winning run ratchets the floor up, a pullback leaves <1 contract of room,
// and the sizer returns 0 -> the symbol silently stops firing ("won itself into a lockout").
// NQ hit this 2026-06; ES hit the IDENTICAL bug 2026-06-08 (only $175 room, no trades since
// yesterday). Give ALL minis the $6,000 buffer so no symbol can lock itself out.
const DD_BY_SYMBOL = { 'NQ=F': 6000, 'ES=F': 6000, 'CL=F': 6000, 'GC=F': 6000,
                       'MNQ=F': 6000, 'MES=F': 6000, 'MCL=F': 6000, 'MGC=F': 6000 };
function _ddDefault(symbol) { return DD_BY_SYMBOL[symbol] || DRAWDOWN_LIMIT; }
...
const CONTRACT_SPECS = {
  // ── Minis ──
  'NQ=F':  { name: 'E-mini Nasdaq-100',  tickSize: 0.25, tickVal:  5.00, pointVal:   20.00, commRound: 4.00, maxContracts: 10, family: 'NQ', isMicro: false },
  'ES=F':  { name: 'E-mini S&P 500',     tickSize: 0.25, tickVal: 12.50, pointVal:   50.00, commRound: 4.00, maxContracts: 10, family: 'ES', isMicro: false },
  'CL=F':  { name: 'Crude Oil',          tickSize: 0.01, tickVal: 10.00, pointVal: 1000.00, commRound: 4.00, maxContracts: 10, family: 'CL', isMicro: false },
  'GC=F':  { name: 'Gold',               tickSize: 0.10, tickVal: 10.00, pointVal:  100.00, commRound: 4.00, maxContracts: 10, family: 'GC', isMicro: false },
...
function _buildDefaultAccount(symbol) {
  return {
    ...
    drawdownFloor: INITIAL_BALANCE - _ddDefault(symbol),
    activePosition: null,
    status: 'Active',
    passed: false,
    mode: 'Evaluation',
    ...
    drawdownAmount: _ddDefault(symbol),
...
function _sizePosition(acc, spec, stopDistance, direction) {
  ...
  if (remainingDD <= 0) return 0; // no room at all — refuse trade

  // HONOR the operator's chosen contract count (userMaxContracts). Take up to
  // effectiveMax contracts as long as the COMBINED stop-loss still fits inside
  // the account's remaining drawdown room — so a full stop-out can't breach the
  // floor. (Changed 2026-06-02 at owner request: dropped the old 25%-of-room
  // throttle that was capping NQ to 1 contract when the user had chosen 2.)
  const maxByRoom = Math.floor(remainingDD / dollarStopPerContract); // # of stops that fit in room
  const qty = Math.min(effectiveMax, maxByRoom);
  if (qty >= 1) return qty;
  // Even one full stop doesn't fit the room → allow 1 only if it just fits, else refuse.
...
// ── Tighter NQ breakeven (Settings checkbox, default OFF) ─────────────────────
// When enabled, NQ-family BE arms (and the trail starts, since trailTriggerPrice
// == beTriggerPrice) at a FIXED tick distance instead of 0.8*ATR — so it engages
// earlier. SL/TP are untouched. env TIGHT_BE_NQ_TICKS overrides for backtests.
// Backtest 365d (trail dist held at the live 1.0 ATR so the trailing-stop artifact
// is constant): BE@25t lifted NQ PF 1.28->1.45, net +$152k, MaxDD -31%, monotonic
// 25-35t. Direction robust; absolute magnitude runs optimistic (bar-based trail).
const TIGHT_BE_FILE = path.join(__dirname, '..', 'models', 'tight_be_nq.json');
let _tbeCache = null, _tbeCacheMs = 0;
function _loadTightBeNq() {
  if (process.env.TIGHT_BE_NQ_TICKS != null) {
    const t = parseFloat(process.env.TIGHT_BE_NQ_TICKS);
    return { enabled: t > 0, beTicks: t };
  }
  if (_tbeCache && (Date.now() - _tbeCacheMs) < 5000) return _tbeCache;
  try {
    const j = JSON.parse(fs.readFileSync(TIGHT_BE_FILE, 'utf-8'));
    _tbeCache = { enabled: j.enabled === true, beTicks: parseFloat(j.beTicks) || 0 };
  } catch (e) { _tbeCache = { enabled: false, beTicks: 0 }; }
  _tbeCacheMs = Date.now();
  return _tbeCache;
}

// Per-session position-size ceiling (models/session_sizing.json), e.g. {"NQ":{"RTH":1}}.
// Used to cap NQ to 1 contract in RTH so an RTH stop (~$833) fits inside the smaller RTH
// daily-loss budget — pairs with the session-split cap that gives RTH its own budget.
// Hot-reloaded (5s). Returns {} on any error (no cap). Keyed by mini family (NQ/ES/...).
const SESSION_SIZE_FILE = path.join(__dirname, '..', 'models', 'session_sizing.json');
let _ssCache = null, _ssCacheMs = 0;
function _loadSessionSizing() {
  if (_ssCache && (Date.now() - _ssCacheMs) < 5000) return _ssCache;
  try { _ssCache = JSON.parse(fs.readFileSync(SESSION_SIZE_FILE, 'utf-8')); }
  catch (e) { _ssCache = {}; }
  _ssCacheMs = Date.now();
  return _ssCache;
}
...
function prepareEntry(symbol, direction, entryPrice, strategy, atr, sessionRegime, manualQty = null) {
  ...
  // Per-session size ceiling (e.g. NQ RTH -> 1 contract) so per-trade risk fits the
  // session's daily-loss budget. Skipped for manual operator qty.
  if (manualQty == null) {
    const _sessSize = _loadSessionSizing();
    const _famK = familyMiniSymbol(symbol).replace('=F', '');
    const _sessCode = (sessionRegime && (sessionRegime.code || sessionRegime.session)) || null;
    const _ceil = _sessSize[_famK] && _sessCode ? _sessSize[_famK][_sessCode] : null;
    if (_ceil != null && _ceil >= 1 && qty > _ceil) qty = _ceil;
  }
  ...
  let beTriggerPrice    = entryPrice + (sessionRegime.atrBreakevenMultiplier * atr * (direction === 'Long' ? 1 : -1));
  // Tighter NQ breakeven (checkbox, default OFF): fixed-tick BE arms earlier than 0.8*ATR.
  const _tbe = _loadTightBeNq();
  if (_tbe.enabled && _tbe.beTicks > 0 && (symbol === 'NQ=F' || symbol === 'MNQ=F')) {
    beTriggerPrice = entryPrice + (_tbe.beTicks * spec.tickSize * (direction === 'Long' ? 1 : -1));
  }
...
function updateLiveMetricsFromNT8(symbol, balance, realized, unrealized, positionInfo) {
  ...
  if (hasPosition) {
    ...
    acc.activePosition = null;
    // A position just closed (we were holding one, NT8 now reports Flat). NT8 only
    // streams AGGREGATE realized P&L — there is no per-trade feed — so reconstruct
    // the closed-trade record HERE from the realized delta + the position we held.
    // This is the sole per-closed-trade source (the paper engine was removed) and it
    // feeds the dashboard "Today's Performance" + "Recent Trade History" panels,
    // which were empty because nothing ever recorded a trade.
    if (prevPosition) {
      const tradePnL = realized - prevRealized;
      try {
        const q       = Math.abs(prevPosition.qty) || 1;
        const pv      = spec ? spec.pointVal : 0;
        const dirSign = prevPosition.direction === 'Short' ? -1 : 1;
        const exitPrice = (pv > 0 && q > 0 && prevPosition.entryPrice)
          ? +(prevPosition.entryPrice + (tradePnL / (pv * q)) * dirSign).toFixed(2)
          : null;
        const rec = {
          symbol,
          direction:    prevPosition.direction,
          qty:          q,
          entryPrice:   prevPosition.entryPrice || null,
          exitPrice,
          profit:       +tradePnL.toFixed(2),
          pnl:          +tradePnL.toFixed(2),
          entryTime:    prevPosition.entryTime || new Date().toISOString(),
          exitTime:     new Date().toISOString(),
          strategyUsed: prevPosition.strategyUsed || 'NT8 Live',
          reason:       'NT8 close',
          accountNumber: acc.accountNumber || null
        };
        if (!Array.isArray(portfolio.history)) portfolio.history = [];
        portfolio.history.unshift(rec);                       // newest first
        if (portfolio.history.length > 300) portfolio.history.length = 300;
        console.log(`[PaperEngine] recorded NT8 close: ${symbol} ${rec.direction} x${q} pnl=$${rec.profit}`);
      } catch (e) { console.warn('[PaperEngine] trade-record build failed:', e.message); }
      // Keep the optional external callback too (unused today, but harmless)
      if (_onPositionClose) {
        try { _onPositionClose(symbol, prevPosition, tradePnL, realized); } catch (e) { /* non-fatal */ }
      }
    }
  }
...
// Re-anchor the trailing drawdown to CURRENT equity — restores the full DD buffer
// without touching balance, realized P&L, or trade history. Use when the trailing
// floor has ratcheted up close to balance (buffer critical) but the account is fine.
// Sets peakEquity = current equity and floor = equity - drawdownAmount (Standard mode
// keeps floor 0). Per-symbol; does NOT clear the daily-loss cap.
function resetDrawdown(symbol) {
  const acc = portfolio.accounts[symbol];
  if (!acc) return { ok: false, error: 'unknown symbol: ' + symbol };
  const ddAmt  = _getDrawdownAmount(acc);
  const equity = acc.balance + (acc.unrealizedPnL || 0);
  acc.peakEquity    = equity;
  acc.drawdownFloor = (acc.mode === 'Standard') ? 0 : (equity - ddAmt);
  savePortfolioState();
  const buffer = acc.balance - acc.drawdownFloor;
  return { ok: true, symbol, peakEquity: acc.peakEquity, drawdownFloor: acc.drawdownFloor,
           drawdownAmount: ddAmt, bufferRestored: buffer };
}
