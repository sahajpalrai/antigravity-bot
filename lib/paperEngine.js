...
function updateLiveMetricsFromNT8(symbol, balance, realized, unrealized) {
  const acc = portfolio.accounts[symbol];
  if (!acc) return false;
  ...
  // Directly update activePosition in the portfolio state to match NT8 exactly!
  if (Math.abs(unrealized) > 0.01) {
    if (!acc.activePosition) {
      acc.activePosition = {
        direction: unrealized > 0 ? 'Long' : 'Short',
        entryPrice: 0,
        qty: spec ? spec.maxContracts : 2,
        stopLoss: 0,
        takeProfit: 0,
