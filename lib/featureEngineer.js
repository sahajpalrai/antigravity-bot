...
// ─── Track C: VWAP + intraday context helpers ──────────────────────────────
// Added 2026-05-28. These helpers are used only by buildFeatures() -- they
// are pure and have no side effects.

// 80-bar rolling VWAP (volume-weighted typical price).
// 5 min x 80 bars = ~6.7 hr ≈ one full RTH session.
function _rollingVWAP(candles, lookback) {
...
// Extract YYYY-MM-DD from a candle timestamp string (PT-offset CSV or ISO).
function _dateOf(ts) {
  if (typeof ts === 'string') return ts.slice(0, 10);
  return new Date(ts).toISOString().slice(0, 10);
}

// First bar of the current trading day: scan back from the last candle
// until the date changes. Returns the close of that first bar.
// Falls back to 78 bars ago if no boundary found (single-day buffer).
function _firstBarOfDayClose(candles) {
  const todayDate = _dateOf(candles[candles.length - 1].time);
  for (let i = candles.length - 2; i >= 0; i--) {
    if (_dateOf(candles[i].time) !== todayDate) {
      return candles[i + 1].close;  // first bar of today
    }
  }
  return candles[Math.max(0, candles.length - 78)].close;
}

// Overnight gap: first open of today vs. last close of yesterday, / ATR.
// Returns 0 if the candle buffer does not straddle a day boundary.
function _gapAtr(candles, atrVal) {
  if (atrVal <= 0) return 0;
  const todayDate = _dateOf(candles[candles.length - 1].time);
  let firstTodayIdx    = candles.length - 1;
  let lastYesterdayIdx = -1;
  for (let i = candles.length - 2; i >= 0; i--) {
    const d = _dateOf(candles[i].time);
    if (d === todayDate) {
      firstTodayIdx = i;
    } else {
      lastYesterdayIdx = i;
  ...
  if (lastYesterdayIdx === -1) return 0;
  return (candles[firstTodayIdx].open - candles[lastYesterdayIdx].close) / atrVal;
}
...
function buildFeatures(candles, opts = {}) {
  ...
  const features = {
    ...
    // ── TRACK C: INTRADAY CONTEXT FEATURES (positions 28-31) ───────────────
    // Added 2026-05-28. Existing deployed models (trained before this commit)
    // only reference positions 0-27 by index -- appending here keeps them valid.
    // New models trained after this commit gain VWAP + intraday context.
    //
    // Most impactful for NQ/ES RTH where mean-reversion to VWAP is a dominant
    // intraday pattern, and gap-direction predicts early RTH continuation.

    // VWAP deviation: (close - 80-bar VWAP) / ATR.
    // Positive = above VWAP (bullish bias), negative = below (bearish bias).
    // More predictive than EMA distance because VWAP weights by VOLUME.
    vwap_dev_atr: (() => {
      const v = _rollingVWAP(candles, 80);
      return (v > 0 && a > 0) ? (last.close - v) / a : 0;
    })(),

    // Intraday momentum: (close - first_bar_of_day_close) / ATR.
    // Captures "how extended is today's move already?"
    intraday_mom: a > 0 ? (last.close - _firstBarOfDayClose(candles)) / a : 0,

    // Overnight gap: (first_open_today - last_close_yesterday) / ATR.
    // Positive = gapped up, negative = gapped down.
    gap_atr: _gapAtr(candles, a),

    // Volume trend: mean of last 5 volumes / mean of last 20 volumes.
    // > 1.0 = volume expanding (participation growing).
    // < 1.0 = volume contracting (exhaustion signal).
    vol_trend: (() => {
      const v5  = (vols.slice(-5).reduce((s, v) => s + v, 0) / 5)  || 1;
      const v20 = (vols.slice(-20).reduce((s, v) => s + v, 0) / 20) || 1;
      return v5 / v20;
    })()
  };
