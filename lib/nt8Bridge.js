...
function _flushPending(symbol) {
  ...
  if (!completed) return;
  _pushCandle(symbol, completed);          // feeds in-memory decision buffer
  _appendCsvRow(symbol, completed);        // keeps training data fresh
  if (_onBarCallback) {
...
function broadcastBrainState(state) {
  ...
  try {
    ...
    clients.forEach(c => { try { c.write(buf); } catch (e) {} });
  } catch (e) {
