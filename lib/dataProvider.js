...
async function fetchRecentCandles(symbol, interval = '1m', range = '1d') {
  ...
  try {
    ...
    if (candles && candles.length >= 30) {
      candles.source = 'Yahoo Finance API';
      return candles;
    }
  } catch (err) {
  ...
  // Fallback: If Yahoo Finance fails or returns insufficient candles, read from the local 3-year CME futures seed files
  try {
    const localData = loadLocalNT8Data(symbol);
    if (localData && localData.length >= 100) {
      console.log(`[DataProvider] Yahoo Finance feed inactive. Slicing local historical seed data for ${symbol}. Returning last 350 candles.`);
      const sliced = localData.slice(-350);
      sliced.source = 'Local NinjaTrader 8 Export';
      return sliced;
    }
  } catch (e) {
    console.error(`[DataProvider] Local historical fallback failed for ${symbol}:`, e.message);
  }

  return [];
}

// In-memory cache for parsed historical CSV data to prevent massive thread-blocking I/O
...
function loadLocalNT8Data(symbol) {
  if (localDataCache[symbol]) {
    return localDataCache[symbol];
  }
