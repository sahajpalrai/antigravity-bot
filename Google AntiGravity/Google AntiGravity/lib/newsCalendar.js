...
async function getNewsTradingSuspension() {
  ...
  for (const event of cachedEvents) {
    ...
    // Check if within [-5 minutes, +5 minutes] of the event
    if (diff >= -fiveMinutesInMs && diff <= fiveMinutesInMs) {
      suspensionActive = true;
      activeReason = `Suspended for High Impact Event: ${event.title} (${event.country})`;
    }
